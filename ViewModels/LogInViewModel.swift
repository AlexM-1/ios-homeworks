...
final class LogInViewModel {
    ...
    var biometricType = LocalAuthorizationService().biometricType
    
    
    enum Action {
        case logInButtonTap
        case signUpButtonTap
        case biometricsButtonTap
        ...
        case .biometricsButtonTap:
            ...
            LocalAuthorizationService().authorizeIfPossible { success, error in
                if success {
                    ...
                    self.stopTimer()
                    self.user = TestUserService().user
                    ...
                    DispatchQueue.main.async {
                        self.coordinator.showAlert(title: "Authorization error".localizable, message: error)
                    }
